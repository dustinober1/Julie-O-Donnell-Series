#!/usr/bin/env python3
"""Permanent validation for accepted Book 1 through Chapter 23 plus the Chapter 24 mission lock."""
...
CH24_MANUSCRIPT = "books/book-01/manuscript/chapters/chapter-24.md"
...
CH24_LOCK_BLOB = "260186500b448b5154858f7df47f113dc8d6fbfa"
...
ALLOWED_CHANGED = set(STATE_FILES) | {
    ...
    CH24_LOCK, "tools/validate_book1_chapter24_mission_lock.py",
...
for path, expected_blob in (
    (MANIFEST, EXPECTED_MANIFEST_BLOB), (LOCK, EXPECTED_LOCK_BLOB),
    (REVIEW, EXPECTED_REVIEW_BLOB), (CH24_LOCK, CH24_LOCK_BLOB),
):
    if not (ROOT / path).is_file() or blob(path) != expected_blob:
...
artifacts23 = [p for p in tracked if "chapter-23" in p.lower() and p.startswith(("books/book-01/control/", "books/book-01/drafts/", "books/book-01/manuscript/"))]
if artifacts23 != [LOCK, REVIEW, CH23]:
    fail(f"unexpected Chapter 23 artifacts: {artifacts23}")
...
if artifacts24 != [CH24_LOCK]:
    fail(f"unexpected Chapter 24 artifacts: {artifacts24}")
for forbidden in (CH24_DRAFT, CH24_MANUSCRIPT, CH24_REVIEW):
...
for phrase in (
    "chapters: 1-23", "accepted_words: 121417", "maximum_words_remaining: 3583",
    "active_chapter_drafts: []", EXPECTED_REVIEW_BLOB, CH23,
    "title: The Terms of Return", "mission_lock: books/book-01/control/48-chapter-24-mission-lock.md",
    "hard_maximum_words: 3583", "no Chapter 24 prose or acceptance review exists.",
):
...
for path in STATE_FILES:
    ...
    lower = text.lower()
    for stale in (
        "chapter 23 acceptance review has not been created", "formal acceptance review not created",
        "formal acceptance review pending", "accepted canon: prologue and chapters 1–22",
        "accepted canon: prologue and chapters 1-22", "no chapter 23 prose or mission lock",
    ):
        if stale in lower:
...
for required in (REVIEW, MANIFEST, CH23, "tools/validate_book1_chapter23.py", CH24_LOCK, "tools/validate_book1_chapter24_mission_lock.py"):
    if required not in changed:
        fail(f"required synchronized change missing: {required}")
for path in changed:
...
if subprocess.run(["git", "diff", "--check", BASE, "--"], cwd=ROOT).returncode:
    fail("git diff --check")
print("PASS: accepted Book 1 through Chapter 23 is protected and Chapter 24 is mission locked without prose")
