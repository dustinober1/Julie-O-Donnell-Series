# 23. WORD BUDGET AND PROVISIONAL ACT III ARCHITECTURE

## Status

**Approved production planning.** This document controls target length and sequence discipline. It is not story canon. Individual chapter functions become controlling only through chapter-specific mission locks and accepted prose.

## Word budget

| Measure | Words |
|---|---:|
| Minimum finished length | 100,000 |
| Planning target | 112,500 |
| Maximum finished length | 125,000 |
| Accepted manuscript through Chapter 16 | 87,610 |
| Remaining to planning target | 24,890 |
| Remaining to minimum | 12,390 |
| Remaining before maximum | 37,390 |

Chapter 13 contributes 6,175 accepted words. Chapter 14 contributes 5,763 accepted words. Chapter 15 contributes 5,993 accepted words. Chapter 16 contributes 6,024 accepted words. Prologue and Chapters 1–12 contribute 63,655 accepted words under the same whitespace-delimited method.

Accepted word counts do not change during mission planning, drafting, or review. They change only when prose is formally accepted and promoted. The Chapter 17 first draft is tracked separately at **5,888 words**.

## Planning principle

The remaining accepted manuscript after Chapter 16 must land between **12,390 and 37,390 new accepted words** to remain inside the 100,000–125,000 target range. The planning target requires **24,890** additional accepted words.

No chapter receives words solely to satisfy the target. Every scene must advance at least one of:

- threat;
- evidence;
- character decision;
- relationship consequence;
- institutional consequence;
- geopolitical consequence; or
- series setup that does not replace Book 1 payoff.

## Provisional remaining structure

The later rows below remain high-level, provisional production planning. They are not a complete controlling outline and may be changed only through later chapter-specific mission locks.

| Unit | Provisional function | Planning range |
|---|---|---:|
| Chapter 13 — accepted | Carrier-stream observation; custody distinction preserved; Hartwell challenge reached | 6,175 actual |
| Chapter 14 — accepted | Independent MPD/Hartwell witness line; hard abort into scene control | 5,763 actual |
| Chapter 15 — accepted | Preserve a linked counterrecord through lawful detention, medical separation, and interim MPD evidence transfer; bounded Sharma reconstruction consequence | 5,993 actual |
| Chapter 16 — accepted | Named preservation sponsorship, Marcus correction, bounded allied acknowledgment | 6,024 actual |
| Chapter 17 — first draft, non-canon | First independent MPD-controlled single-package technical examination | 5,888 draft |
| Chapter 18 and later | Provisional only; no individual lock | Not controlling |

## Sequence rules

1. Chapters 13–16 are accepted and control the 08:15:52 EDT / 17:45:52 IST opening state for Chapter 17.
2. Chapter 17 is controlled by `34-chapter-17-mission-lock.md`; its first draft lives only at `../drafts/chapter-17.md` and remains non-canon.
3. Chapter 17 drafted events may not control Chapter 18 or later prose unless the formal acceptance gate issues **ACCEPT** and synchronized promotion is completed.
4. K-17 must return to the foreground before the climax.
5. Independent evidence custody must develop before the final confrontation; it cannot appear as a convenient last-page solution.
6. Marcus’s on-record correction has occurred; consequences remain required.
7. Chen and Mercer must remain institutionally grounded; neither becomes a secret ally by convenience.
8. The climax must resolve the immediate operation, not merely expose another layer.
9. The resolution chapter must spend enough time on legal, emotional, public, and series consequences.
10. The epilogue is optional and must obey the anti-cliffhanger rule in the ending contract.
11. Chapter 18 and later functions remain provisional until individually mission locked.

## Locked Chapter 17 boundaries and draft result

- Opens exactly at 08:15:52 EDT / 17:45:52 IST.
- Uses Julie primary plus one bounded Special Agent Leila Grant examination-room movement.
- Opens only `MPD-901446`.
- Keeps the other six packages sealed and unopened.
- Keeps MPD physical custody throughout.
- Uses read-only independent tooling; no biometric, private-key signing act, live network, Apex credential, or second package.
- Ends at 09:12:52 EDT / 18:42:52 IST.
- Draft length: **5,888 words**, within the 5,600–6,300 planning range and near the 5,950 preferred center.
- Does not identify the Hartwell presenter or exact serial.
- Does not decrypt WSS-4.
- Does not define Phase B or confirm K-17 access.
- Does not resolve final federal custody, guilt, innocence, immunity, or public vindication.
- Does not draft the remainder of Act III.
- Remains unaccepted until the formal gate issues **ACCEPT**.

## Expansion and compression controls

### Expand when

- a crucial evidence distinction is not dramatized;
- a recurring character’s choice lacks setup;
- the K-17 and Washington threads are not causally connected;
- the public narrative fracture happens offstage;
- Marcus’s corrective action lacks cost; or
- Julie’s ending role is asserted rather than chosen.

### Compress when

- characters repeat already-established evidence;
- another chase reproduces an earlier escape without new consequence;
- technical explanation does not alter a decision;
- an antagonist scene says what the evidence already shows;
- a series seed competes with Book 1 closure; or
- a chapter has more than one dominant mission.

## Remaining high-level obligations

The remaining Act III must still provide enough space for:

- Washington/field reconnection;
- Phase B clarification;
- K-17 immediate outcome;
- Hartwell exact-record payoff;
- WSS local-audit fate;
- independent technical examination beyond the first bounded result where necessary;
- Chen and Mercer choices;
- Hackett and Price payoffs;
- consequences of Marcus's formal correction;
- meaningful Vance consequence;
- public-narrative fracture;
- climax and geopolitical stabilization;
- character/legal aftermath; and
- a bounded series-facing ending.

The exact distribution across later chapters remains provisional and must be locked one chapter at a time.

## Final length review

At the end of the first full draft:

1. Recount accepted words only.
2. Audit each chapter’s dominant function.
3. Cut repeated explanation before cutting consequence.
4. Add missing decision or aftermath scenes before adding procedural detail.
5. Keep the final manuscript within 100,000–125,000 words.

---

## Historical Chapter 15 and Chapter 16 updates

- Chapter 15 accepted contribution: **5,993** words.
- Chapter 16 accepted contribution: **6,024** words.
- Accepted Act III subtotal through Chapter 16: **23,955** words.
- The Chapter 16 mission range was satisfied and the chapter was accepted after one surgical equal-count POV-firewall repair.

## Chapter 17 first-draft word-budget update — 2026-07-13

| Measure | Words |
|---|---:|
| Accepted manuscript through Chapter 16 | 87,610 |
| Remaining to minimum 100,000 | 12,390 |
| Remaining to planning target 112,500 | 24,890 |
| Remaining before maximum 125,000 | 37,390 |
| Accepted Act III subtotal, Chapters 13–16 | 23,955 |
| Chapter 17 first draft | 5,888 |
| Chapter 17 planning range | 5,600–6,300 |
| Chapter 17 preferred center | approximately 5,950 |

- Chapter 17 is mission locked as **The First Examination** at `34-chapter-17-mission-lock.md`.
- One first draft exists at `../drafts/chapter-17.md`; it is **unaccepted and non-canon**.
- Draft endpoint: **09:12:52 EDT / 18:42:52 IST**.
- Accepted words, inventory, and endpoint remain unchanged at **87,610 words** and **08:15:52 EDT / 17:45:52 IST**.
- Chapter 18 and later functions remain provisional and are not individually expanded or locked by this update.

Immediate next gate: conduct the formal Chapter 17 acceptance review. Do not draft Chapter 18 or create a complete remainder-of-Act-III outline.
