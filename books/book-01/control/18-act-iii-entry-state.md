# 18. ACT III ENTRY STATE

## Resolved pre-Act-III continuity state

The Chapter 5-to-6 chronology and physical handoff were repaired and integrated on 2026-07-12 in commit `__INTEGRATION_COMMIT_SHA__`. It is no longer an Act III blocker. Chapter 13’s precise mission, abort condition, and POV/cutaway choice remain planning decisions and are not locked by this repair.

## Exact launch state

**Time:** Approximately 07:18 EDT / 16:48 IST
**Location:** Downtown Washington, exact street unstated
**Vehicle:** PCF-27
**Characters present:** Julie, Marcus, Elias

## Physical condition

### Julie

- Driving with left hand.
- Right wrist braced and functionally weak.
- Pain worsened by every road seam and turn.
- Exhausted and publicly recognizable.

### Marcus

- Passenger seat.
- Reopened scalp wound with blood to jaw.
- Rib pain prevents full breath.
- Swollen right thigh and unreliable leg.
- Taped damaged boot.
- Limited balance and mobility.

### Elias

- Cargo floor.
- Left hip injury.
- Bandaged, bleeding right index finger.
- Dizziness and exhaustion.
- Torn office shirt; no rain shell.

## Evidence and custody

### Marcus

- Primary aluminum case.
- 136 sealed files.
- 47 incomplete files.
- 311 exclusions manifest.
- Recovery cartridge.
- Paper custody log.
- Dead transponder.

### Elias

- Administrator-token board.
- Partition A.
- Partition B with valid WSS controller seal.

### External copies

- WSS-4 local record at Northbridge.
- Chen directives at Apex.
- Sharma/Qureshi image and Indian fire-control records at Arjun.

## Pursuit status

- Van plate and direction confirmed.
- Metropolitan Police active.
- Apex Protective Services active.
- Regional and federal channels have the lookout.
- Public images circulating.
- Hartwell perimeter is already tightening.
- Sterling’s motorcade has controlled signals and secure access.

## Public narrative

Julie is the alleged architect of an expanding insider attack against Apex and Northbridge. Marcus is armed accomplice; Elias is hostage or coerced employee.

## Known Hartwell timing

07:46:00–07:48:30 EDT.

## Known Phase B timing

Authorization deadline 07:49 EDT.

## Known masking timing

07:48–07:54 EDT.

## Known K-17 status

- Security Line One passed.
- Relay access not confirmed at 07:08.
- Physical status unknown at 07:18.

## What Julie intends to accomplish

At minimum, Julie intends to move toward the next physical authentication point rather than continue fleeing. The exact Chapter 13 operational plan is not yet canon.

## What Julie can prove

- Payload 88 was synthetic.
- It was operationally deployed.
- Vance-registered executive authority controlled the bridge.
- Elias’s physical token did not sign the original deployment.
- Julie’s recovery was an authenticated provenance correction.
- WSS-4 hosted K17-PHASE-B.
- Tariq field authority and Sterling-office SSO participated.
- SSO-NS-004 is expected at Hartwell.
- Relay access remained unconfirmed.
- Phase B and masking remain pending.

## What Julie cannot prove

- Sterling personally held or used SSO-NS-004.
- The identity of the current custodian.
- Sterling’s exact command role.
- Message plaintext.
- Current K-17 position.
- Phase B’s purpose.
- Whether no Indian rounds were fired.
- Vance’s complete relationship with Sterling or Tariq.
- Current preservation of the WSS local record.

## What constitutes success

A useful Act III success should include one or more of:

- Preventing or delaying Phase B authorization.
- Capturing independently verifiable physical-custody evidence for SSO-NS-004.
- Preserving the Hartwell authentication event with neutral witnesses or systems.
- Reaching an independent official with both Apex and WSS evidence.
- Preventing K-17 exploitation.
- Fracturing the public narrative without exposing harmful raw intelligence.
- Keeping all three protagonists and the primary evidence alive.

## What constitutes failure

- Phase B authorized.
- Second masking/reconstruction profile executes.
- K-17 logs or access are successfully manipulated.
- Evidence destroyed or separated into hostile custody without redundancy.
- Julie, Marcus, or Elias killed or incapacitated.
- A civilian or honest officer is harmed.
- Julie’s actions validate Sterling’s portrayal.
- The signer disappears without a physical-custody record.

## Ethical boundaries still in force

- No civilian casualties.
- No hostage-taking.
- No assassination.
- No firearm escalation without a new, immediate life threat.
- No ramming through people.
- No indiscriminate public dump.
- No assumption that office registration proves human guilt.
- No destruction of unrelated systems or records.
- Evidence before revenge.
- Preserve raw truth and uncertainty.

## Immediate practical obstacles

- Only 28 minutes to window opening.
- Known van and active plate.
- Hartwell lockdown.
- No credential or sponsor.
- No known Hartwell layout.
- Severe injuries.
- No secure communications channel.
- No confirmed independent recipient.
- Active police and Apex response.
- Public facial recognition.
- Motorcade and traffic restrictions.
- Messages encrypted.
- K-17 status unknown.

## Approved Act III Structural Direction

The revised Act II structure establishes only a broad, still-valid direction:

- Act III begins with Julie moving toward the conspiracy’s secure political center rather than continuing to escape.
- Hartwell is the next physical objective.
- The missing proof concerns physical custody or independently verifiable use of the office signer.
- Julie’s objective remains authenticated evidence, not revenge.
- Vance, Sterling, and Tariq must retain distinct motives.
- Chen’s fracture with Vance may create evidence or procedural opportunity, but she is not yet an ally.
- The field crisis at K-17 remains live.

The older review stated as fact that Sterling carried the device and that Tariq was less than an hour from K-17. The accepted manuscript supersedes those claims: the physical custodian is unknown, and relay access was not confirmed.

No complete approved Act III outline or locked Chapter 13 title was found.

## Decisions the author should make before drafting

1. The precise Hartwell objective.
2. The intended physical custodian of SSO-NS-004.
3. Whether Chapter 13 reaches the perimeter, enters it, or ends before entry.
4. Whether K-17 or Sharma receives a parallel scene.
5. What immediate independent witness or record can authenticate the Hartwell event.
6. Whether the trio stays together or splits evidence and action roles again.
7. What abort condition Julie sets.
8. Whether Phase B’s meaning is revealed at Hartwell or in the field.

These are planning decisions, not canon facts.

---
