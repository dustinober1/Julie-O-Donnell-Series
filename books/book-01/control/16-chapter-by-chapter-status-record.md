# 16. CHAPTER-BY-CHAPTER STATUS RECORD

The accepted-manuscript inventory in `../ACCEPTED_MANUSCRIPT.yaml` controls acceptance status.

## Prologue — Six Years Ago

- **Status:** Accepted.
- **Purpose:** Establish Julie's competence, the original Argus wound, civilian cost, four-second delay, Marcus's betrayal, and the official-record theme.
- **Ending state:** The machine was wrong; the record says Julie was late.
- **Series carry:** Four-second delay, Hargrove, Marcus's testimony, and the possibility of a larger repeated method.

## Chapter 1 — The Official Version

- **Status:** Accepted.
- **Purpose:** Reestablish Julie's civilian life, bring Marcus back, introduce the new threat, and place her under supervised Apex control.
- **Ending hook:** The machine is lying in the same voice.

## Chapter 2 — The Poisoned Feed

- **Status:** Accepted.
- **Purpose:** Establish Payload 88, Elias's framing, and the first evidence-capture opportunity.
- **Ending hook:** The room is locked and the transfer is incomplete.

## Chapter 3 — The Exit Protocol

- **Status:** Accepted.
- **Purpose:** Escape with partial evidence and introduce Sharma as the independent human decision-maker in Kashmir.
- **Evidence:** Partial case and Sharma's local anomaly record.

## Chapter 4 — Burn Notice

- **Status:** Accepted.
- **Purpose:** Analyze the partial evidence, show Vance framing Elias, reject unsafe surrender, and choose a bounded return to Apex.
- **Continuity:** No firearms; gray coveralls; Ford remains outside the perimeter.

## Chapter 5 — The Second Clock

- **Status:** Accepted.
- **Continuity repair:** Integrated in `f6d49cbeae2b2f23daac55dc0bacfeb040428f5f`.
- **Time:** 15:41 EDT through approximately 04:52 EDT the next day, with India crosscuts.
- **Purpose:** Establish the two-stage release architecture, sustain pressure without recovery, reenter Apex, reunite the trio, and deliver the exact L3-7 handoff.
- **Ending:** Hydraulic shutter L3-7 descends above Elias and Julie while Marcus remains last.

## Chapter 6 — The Descent

- **Status:** Accepted.
- **Time:** 04:52–04:54:47 EDT.
- **Purpose:** Trap the trio below Apex and make enclave access the only route to prevent the 05:00 release.
- **Injuries begun:** Julie's wrist; Marcus's boot and ribs; Elias's hip.

## Chapter 7 — The Human Key

- **Status:** Accepted.
- **Time:** 04:55–04:58:11 EDT.
- **Purpose:** Force Elias's voluntary choice, authenticate access realistically, and bring the trio through the gate.
- **Character change:** Elias becomes a voluntary whistleblower; Chen refuses Vance's lethal-force escalation.

## Chapter 8 — The 05:00 Abort

- **Status:** Accepted.
- **Time:** 04:58–05:01 EDT / approximately 14:28–14:31 IST.
- **Purpose:** Stop the immediate catastrophe, preserve uncertainty, reveal K-17 movement, and escalate Vance.
- **Reader/trio distinction:** The reader knows Sharma aborted and no rounds were fired; the trio does not.

## Chapter 9 — The Life-Safety Override

- **Status:** Accepted.
- **Time:** 05:01–05:14:36 EDT.
- **Purpose:** Escape with evidence, establish the public false narrative, and place the trio in a known vehicle.
- **Injuries:** All materially worsen.

## Chapter 10 — The Capital Connection

- **Status:** Accepted.
- **Time:** 05:15–06:25 EDT.
- **Purpose:** Stabilize injuries, authenticate the evidence, connect Vance to Northbridge, and locate the 07:08 schedule.

## Chapter 11 — Going Offensive

- **Status:** Accepted.
- **Time:** 06:25–06:50 EDT.
- **Purpose:** Define the morally bounded Northbridge operation and formalize evidence roles.
- **Character change:** Julie commands; Elias volunteers; Marcus accepts exterior custody.

## Chapter 12 — The Sterling Trap

- **Status:** Accepted.
- **Time:** 06:50–07:18 EDT.
- **Purpose:** Conduct the first offensive operation, capture stronger proof, trigger the trap, and launch Act III.
- **Major proof:** Tariq field authority, Sterling-office SSO-NS-004, Hartwell timing, Phase B deadline, and Masking Window Two.
- **Limit:** The human custodian of SSO-NS-004 remains unproved.

## Chapter 13 — The Carrier Stream

- **Status:** Accepted canon.
- **Accepted path:** `../manuscript/chapters/chapter-13.md`.
- **Acceptance review:** `27-chapter-13-acceptance-review.md` — **ACCEPT**.
- **Exact word count:** 6,175 whitespace-delimited Markdown words.
- **Time:** Approximately 07:18–07:46 EDT / 16:48–17:16 IST.
- **Purpose:** Complete the moving physical-custodian identification operation, establish continuous or recoverable observation, and reach the Hartwell challenge without inventing the human custodian.
- **Hard-abort result:** Both locked conditions pass at 07:42:30.
- **Endpoint:** Garage level six at 07:46:00; three-person support stream and compact black case observed; human custodian unresolved.

## Chapter 14 — The Witness Line

- **Status:** Accepted canon.
- **Accepted path:** `../manuscript/chapters/chapter-14.md`.
- **Mission lock:** `28-chapter-14-mission-lock.md`.
- **Acceptance review:** `29-chapter-14-acceptance-review.md` — **ACCEPT**.
- **Reviewed blob:** `78f7fff02cd271fecbc94f7daf7151dbebbd5c6d`.
- **Exact word count:** 5,763 whitespace-delimited Markdown words.
- **Time:** 07:46:00–07:49:32 EDT / 17:16:00–17:19:32 IST.
- **Dominant function:** Create the independent MPD/Hartwell witness line and abort the observation mission when the west route closes.
- **Endpoint:** Garage level six beside powered-down PCF-27 under MPD scene control; evidence remains separately held; no transfer receipt complete.
- **Accepted manuscript total:** 75,593 words.

## Chapter 15 — The Split Record

- **Status:** Accepted canon.
- **Accepted path:** `../manuscript/chapters/chapter-15.md`.
- **Mission lock:** `30-chapter-15-mission-lock.md`.
- **Acceptance review:** `31-chapter-15-acceptance-review.md` — **ACCEPT**.
- **Reviewed blob:** `b8e7e2ae573a6c25ea096121c75acee867f3fad2`.
- **Review prose change:** One capitalization-only copyedit in the body-camera paragraph; word count unchanged.
- **Exact word count:** 5,993 whitespace-delimited Markdown words.
- **Time:** 07:49:32–07:56:40 EDT / 17:19:32–17:26:40 IST.
- **POV:** Julie close third plus one bounded Sharma cutaway.
- **Dominant function:** Complete separate interim MPD custody while preserving the superseding/prior allied-record contradiction.
- **Endpoint:** Seven MPD packages sealed under incident `187463`; Marcus, Elias, and Julie transported separately; no final federal receiver.
- **Accepted manuscript total:** 81,586 words.

## Chapter 16 — The Hold Order

- **Status:** Accepted canon.
- **Accepted path:** `../manuscript/chapters/chapter-16.md`
- **Mission lock:** `32-chapter-16-mission-lock.md`
- **Acceptance review:** `33-chapter-16-acceptance-review.md`
- **Exact words:** **6,024**
- **Reviewed blob:** `dd5249f4b510a9da9ad19ab2902a95ce1e62a1d8`
- **Prose changed during review:** **Yes — one surgical POV-firewall revision in the Sharma cutaway**
- **Opening:** **07:56:40 EDT / 17:26:40 IST**
- **Endpoint:** **08:15:52 EDT / 17:45:52 IST**
- **POV:** Julie → Marcus → Sharma → Julie
- **Verdict:** **ACCEPT**
- **Accepted manuscript total:** **87,610 words**

## Chapter 17 — The First Examination

- **Status:** First draft exists; unaccepted; **non-canon**.
- **Mission lock:** `34-chapter-17-mission-lock.md`.
- **Draft path:** `../drafts/chapter-17.md`.
- **Exact draft word count:** **5,888 whitespace-delimited Markdown words**.
- **Draft opening:** **08:15:52 EDT / 17:45:52 IST**.
- **Draft endpoint:** **09:12:52 EDT / 18:42:52 IST**.
- **Dominant function:** Conduct the first independent, MPD-controlled technical examination of one evidence package and create a signed, reproducible finding without fragmenting the seven-package incident.
- **Named examiner:** Special Agent Leila Grant, Defense Criminal Investigative Service.
- **POV:** Julie primary; one bounded Grant examination-room movement.
- **Package scope:** `MPD-901446` only; the other six packages remain sealed and unopened.
- **Acceptance-gate status:** Not yet conducted.
- **Accepted-manuscript effect:** None; accepted total remains 87,610 and the accepted endpoint remains 08:15:52 EDT / 17:45:52 IST.
- **Next gate:** Conduct the formal Chapter 17 acceptance review.

## Chapter 18 and later

- **Status:** Not drafted and not individually mission locked.
- **Planning:** High-level functions remain provisional in `23-word-budget-and-act-iii-architecture.md`.
- **Rule:** Do not draft Chapter 18 or create a complete remainder-of-Act-III outline during the Chapter 17 acceptance-review session.
