# Book 1 Accepted-Manuscript Status

**Authoritative production state:** [`../../../PROJECT_STATE.yaml`](../../../PROJECT_STATE.yaml)

**Authoritative accepted inventory:** [`../ACCEPTED_MANUSCRIPT.yaml`](../ACCEPTED_MANUSCRIPT.yaml)

## Accepted manuscript

- Prologue
- Chapters 1–17
- Acts I and II complete
- Act III: Chapters 13–17 accepted
- Accepted length: **93,498 words**
- Canon endpoint: **09:12:52 EDT / 18:42:52 IST**
- Active chapter draft: `../drafts/chapter-18.md`
- Book 1 status: not publication-ready

## Accepted Chapter 17

- Title: **The First Examination**
- Path: [`chapters/chapter-17.md`](chapters/chapter-17.md)
- Exact words: **5,888**
- Reviewed blob: `1c4022ebb8d27d8d448f98bcf74fbf09e6e560c1`
- Opening: **08:15:52 EDT / 17:45:52 IST**
- Endpoint: **09:12:52 EDT / 18:42:52 IST**
- Examiner: Special Agent Leila Grant, DCIS
- Scope: `MPD-901446` only
- Custody: all seven packages remain in MPD physical custody
- Reseal: `MPD-SL-551821`; former seal `MPD-SL-551804` retained
- Result: the examined board lacks the original attributed physical signing event; later gate and provenance-reconciliation events remain authenticated
- Limit: no alternative identity path, human operator, innocence, guilt, immunity, admissibility, final receiver, Hartwell result, WSS plaintext, K-17 result, Phase B result, field truth, or public vindication was established

## Chapter 18 draft

- Title: **The Local Record**
- Mission lock: [`../control/36-chapter-18-mission-lock.md`](../control/36-chapter-18-mission-lock.md)
- Draft path: [`../drafts/chapter-18.md`](../drafts/chapter-18.md)
- Status: **complete first draft; unaccepted; non-canon**
- Exact draft words: **4,600**
- Opening: **09:12:52 EDT / 18:42:52 IST**
- Endpoint: **10:01:12 EDT / 19:31:12 IST**
- POV: Major Ananya Sharma only
- Accepted-manuscript effect: none
- Manuscript Chapter 18 file: none
- Chapter 19 prose or mission lock: none

## Current unresolved accepted state

Hartwell presenter and exact serial remain unresolved. WSS-4 plaintext remains unavailable. K-17 access and Phase B remain unresolved in accepted canon. Sterling's hostile public account remains dominant. No final federal receiver exists.

The Chapter 18 draft may not change any accepted fact until a formal review issues an explicit ACCEPT verdict and the exact reviewed prose is promoted with synchronized canon updates.

## Current gate

Conduct the formal Chapter 18 acceptance review. Do not draft Chapter 19 or create a complete remainder-of-Act-III outline.
