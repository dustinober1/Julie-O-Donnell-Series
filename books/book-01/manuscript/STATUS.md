# Book 1 Accepted-Manuscript Status

**Authoritative production state:** [`../../../PROJECT_STATE.yaml`](../../../PROJECT_STATE.yaml)

**Authoritative accepted inventory:** [`../ACCEPTED_MANUSCRIPT.yaml`](../ACCEPTED_MANUSCRIPT.yaml)

## Accepted manuscript

- Prologue and Chapters 1–21
- Acts I and II complete
- Act III: Chapters 13–21 accepted
- Accepted length: **112,091 words**
- Canon endpoint: **12:18:04 EDT / 21:48:04 IST**, secure MPD evidence intake
- Book 1 status: not publication-ready

## Accepted Chapter 21

- Title: **The Borrowed Name**
- Path: [`chapters/chapter-21.md`](chapters/chapter-21.md)
- Exact words: **4,415**
- Reviewed blob: `866d4210b7fc808aef48144a91a58280f38fc99c`
- Mission lock: [`../control/42-chapter-21-mission-lock.md`](../control/42-chapter-21-mission-lock.md)
- Mission-lock blob: `6c92a5764e5c74d88a8325511ae2b0a86b30b356`
- Acceptance review: [`../control/43-chapter-21-acceptance-review.md`](../control/43-chapter-21-acceptance-review.md)
- Verdict: **ACCEPT**
- Opening: **11:26:32 EDT / 20:56:32 IST**, secure MPD evidence intake
- Endpoint: **12:18:04 EDT / 21:48:04 IST**, secure MPD evidence intake
- POV: Julie primary with one bounded Leland Price cutaway
- Result: Price's authentic source-review request is separately authenticated; his active authority ended before the later office request; the later request inherited his identity/reference and added operational routing without a matching authenticated Price act
- Proof limit: the constructor, Sterling personal command, alternative deployment actor, and remote-reconstruction author remain unproved

## Custody and medical state

`SSO-NS-004` remains closed and unused under Martin Vann, Marisol Vega, and seal `LSS-SL-90418`. `MPD-901441` through `MPD-901447` remain sealed, separate, offline, stationary, and in the locked common chest. Source institutions retain their originals. Julie remains detained with a materially unusable right wrist/forearm. Marcus remains in guarded medical care on four liters of oxygen at 92–93 percent saturation. Elias remains in separate guarded medical custody.

## Chapter 22 non-canon draft

- Title: **The Release Record**
- Mission lock: [`../control/44-chapter-22-mission-lock.md`](../control/44-chapter-22-mission-lock.md)
- Mission-lock blob: `9bd255ac7b09a1490dc70be4506ba29183756788`
- Draft path: [`../drafts/chapter-22.md`](../drafts/chapter-22.md)
- Exact draft blob: `034ab496794594427d8409d03e7c6659d41b6a91`
- Exact draft words: **4,716**
- Status: first draft complete, non-canon, formal acceptance review pending
- Opening: **12:18:04 EDT / 21:48:04 IST**, secure MPD evidence intake
- Endpoint: **13:12:44 EDT / 22:42:44 IST**, secure MPD evidence intake
- POV: Julie primary with exactly one bounded Sarah Chen cutaway
- Structure: exactly six causal scenes
- Dominant function: relate the Price borrowed identity, Elias framed identity, failed K-17 local commit, and successful remote reconstruction at mechanism and authority-binding levels; establish a separately corroborated later Vance release act and operational consequence without proving the original deployment hand or Sterling personal command
- Accepted effect: none
- Acceptance review: not created

The draft does not alter `../ACCEPTED_MANUSCRIPT.yaml`, accepted words, the accepted endpoint, accepted prose, or accepted character state. Draft-only records and consequences must not be treated as canon until an explicit formal acceptance verdict authorizes promotion.

## Chapter 23 and later

At least one later chapter remains necessary under the ending contract, but Chapter 23 and later remain undrafted and individually mission unlocked. No Chapter 23 mission lock or complete chapter-by-chapter remainder-of-Act-III outline exists.

## Current gate

Perform only the formal Chapter 22 acceptance review against the exact draft and mission lock. Do not draft or plan Chapter 23, create a complete remainder outline, or promote Chapter 22 without an explicit acceptance verdict.
